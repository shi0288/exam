...
public class MongoRepository<T, V> {
     ...
     * query by ID
     */
    public T findById(V id, String collectionName) {
        if (StringUtils.isEmpty(collectionName)) {
            return template.findById(id, entityClass);
        } else {
            return template.findById(id, entityClass, collectionName);
        }
     ...
     * query And Modify
     */
    public T findAndModify(T t, T u, String collectionName) {
        Query query = this.getQuery(t);
        Update update = this.getUpdate(u);
        return template.findAndModify(query, update, entityClass, collectionName);
    }
     ...
     * query And findOne
     */
    public T findOne(T t, String collectionName) {
        Query query = this.getQuery(t);
     ...
     * query And Remove
     */
    public T findAndRemove(T t, String collectionName) {
        Query query = this.getQuery(t);
        return template.findAndRemove(query, entityClass, collectionName);
    }
     ...
     * query And RemoveAll
     */
    public List<T> findAndRemoveAll(T t, String collectionName) {
        Query query = this.getQuery(t);
        return template.findAllAndRemove(query, entityClass, collectionName);
    }
     ...
     * query ids
     */
    public List<T> findByIds(List<V> ids, Sort.Order[] sorts, String collectionName) {
        Query query = new Query();
        //config query
        query.addCriteria(Criteria.where("_id").in(ids));
        if (sorts != null && sorts.length > 0) {
     ...
     * count
     */
    public Long count(T t, String collectionName) {
        Query query = this.getQuery(t);
     ...
     * exists
     */
    public boolean exists(T t, String collectionName) {
        Query query = this.getQuery(t);
        return template.exists(query, entityClass, collectionName);
    }
     ...
     * remove
     */
    public Integer remove(T t, String collectionName) {
        return template.remove(t, collectionName).getN();
    }
     ...
     * update
     */
    public Integer update(T t, T u,String collectionName) {
        Query query = this.getQuery(t);
        Update update = this.getUpdate(u);
        return template.updateMulti(query, update, entityClass,collectionName).getN();
    }
     ...
     * inc
     */
    public T inc(T t, String key, Integer num,String collectionName) {
        Query query = this.getQuery(t);
        //config update
        Update update = new Update();
        update.inc(key, num);
